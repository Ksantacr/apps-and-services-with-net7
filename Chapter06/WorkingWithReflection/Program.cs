...
using WorkingWithReflection; // Assembly
...
foreach (Type type in types)
{
 ...
 member.GetCustomAttribute<ObsoleteAttribute>();
        ...
        WriteLine("{0}: {1} ({2}) {3}", member.MemberType, member.Name,
            member.DeclaringType?.Name, obsolete is null ? "" : $"Obsolete! {obsolete.Message}");
